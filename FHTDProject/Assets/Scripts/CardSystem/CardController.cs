...
public class CardController : MonoBehaviour, IPointerDownHandler
{
    ...
    public void AddCard(int _iCardId = 0)
    {
        ...
        StartCoroutine(AddCardCoroutine(_iCardId));
    }

    IEnumerator AddCardCoroutine(int _iCardId)
    {
        AbleCardBundle();
        ...
        if (_iCardId == 0)
            ...
            listCardDP[iHandCount].GetComponent<CardDisplay>().UpdateCardInfo(CardManager.instance.GetCardWithID(_iCardId));
