...
public enum BuffType { Airborne, Stern, Stiff, Silent, Attack, Power, MaxHp, MaxMp, PlusMp, Shield, ADDef, APDef, Unspecified, SendDamage, ReciveDamage, AtkSpeed, MoveSpeed, Range, CriticalRatio, CriticalDamage, HealAmount, Taunt, Count }
...
public abstract class Unit : MonoBehaviour
{
    ...
    IEnumerator LevelUpCoroutine(List<Unit> _listSubhero)
    {
        ...
        foreach (var item in _listSubhero)
        {
            ...
            Transform _tfLevelUpEffect = Instantiate(Resources.Load<Transform>("Unit/BasicEffect/LevelUp Effect"));

            _tfLevelUpEffect.gameObject.SetActive(true);
            _tfLevelUpEffect.position = item.currentTile.tilePos;
            _tfLevelUpEffect.LookAt(currentTile.tilePos);
            _tfLevelUpEffect.DOMove(currentTile.tilePos, 1f).OnComplete(() => Destroy(_tfLevelUpEffect.gameObject));
        }
